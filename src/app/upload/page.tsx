...
export default function UploadPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 서버로 전송하여 실행
      const executeResponse = await fetch("/api/transaction/execute", {
        method: "POST",
        ...
        body: JSON.stringify({
          ...
          transactionType: objectid ? "create_work_with_parent" : "create_work",
          metadata: {
            ...
            metadata: {
              ...
              file_type: formData.originalFile.type,
              file_size: formData.originalFile.size,
              tags: formData.tags,
            ...
            fee:
              ...
              1_000_000_000,
            licenseOptions: formData.licenseOption
              ? {
                  rule: formData.licenseOption.rule,
                  price: formData.licenseOption.price * 1_000_000_000,
                  royaltyRatio: formData.licenseOption.royaltyRatio,
                }
              : null,
          },
        }),
      ...
      // --- 1.5. Thumbnail 업로드 (있는 경우) ---
      let thumbnailUrl: string | null = null;
      if (formData.thumbnailFile) {
        console.log("Step 1.5: Uploading thumbnail to Supabase Storage...");
        ...
        if (thumbnailUrl) {
          ...
          // preview_uri 업데이트
          await updateWorkPreviewUri(workObjectId, thumbnailUrl);
        } else {
          console.warn("Thumbnail upload failed, but continuing...");
        }
      }

      // --- 2. 프론트엔드에서 Seal encrypt & Walrus upload ---
      ...
      const blobId =
        walrusResponse?.info?.newlyCreated?.blobObject?.blobId ||
        walrusResponse?.info?.alreadyCertified?.blobId;
      ...
      const signedPublishTx = await signTransaction({ transaction: publishTx });

      const publishResponse = await fetch("/api/transaction/execute", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          signedTransaction: {
            bytes: signedPublishTx.bytes,
            signature: signedPublishTx.signature,
          },
      ...
      if (!publishResult.success) {
        throw new Error(publishResult.error || "Publish transaction failed");
      }
    ...
    <div className="min-w-screen min-h-screen">
      ...
      <div className="pt-32 pb-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          {/* 기존 Upload.tsx의 renderStepContent() 내용을 여기에 복사 */}
                    ...
                    <div className="relative group">
                      <div className="aspect-square rounded-xl overflow-hidden bg-gray-100 border border-gray-200">
                        {formData.thumbnailFile?.type.startsWith("image/") ? (
                          ...
                          />
                        ) : formData.thumbnailFile?.type.startsWith(
                            "video/"
                          ) ? (
                          <video
                            src={formData.thumbnailPreview}
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div className="bg-white rounded-sm border border-gray-300 overflow-hidden">
                  ...
                  <div className="p-4 border-b border-t border-gray-300 bg-gray-50">
                    <Label
                      htmlFor="description"
                      className="text-base font-galmuri"
                    >
                      DESCRIPTION *
                    </Label>
                  </div>
